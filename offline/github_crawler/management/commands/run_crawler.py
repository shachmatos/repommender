...
class Command(BaseCommand):
        ...
        if os.path.exists(BASE_DIR + '/storage/scan_incomplete.csv'):
            while resume not in ['y', 'Y', 'n', 'N']:
                ...
                if resume == 'y' or resume == 'Y':
                    ...
                    do_not_resume = True
        self.get_repositories(threshold, do_not_resume)

    def read_scanned_ids(self, path):
        f = open(path, 'r');
        result = set()
        reader = csv.reader()
        for row in reader:
    ...
    def get_repositories(self, threshold, do_not_resume = False):
        ...
        if os.path.exists(storage_path + 'scan_incomplete.csv'):
            if do_not_resume:
                ...
                resumed = True
                repo_ids = self.read_scanned_ids(repos_scanned_file)
